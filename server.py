...
while True:
    client_socket1, clientIp1 = socket1.accept()
    print("[!] Connection from {}".format(clientIp1))
    file1 = open("{}.jpg".format(clientIp1), "wb")
    image_chunk = client_socket1.recv(4096)

    while image_chunk:
        file1.write(image_chunk)
        image_chunk = client_socket1.recv(4096)
    file1.close()
    client_socket2, clientIp2 = socket2.accept()
    encoded_data = client_socket2.recv(4096)
